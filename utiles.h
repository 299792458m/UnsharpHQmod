				  ...
				  float thres,float MIN,float MAX,bool show,int mode)
{
	...
	memcpy( (BYTE*) dstp+dst_pitch, (BYTE*) row1, rowsize);
	
	dstp=dstp+2*dst_pitch;
	int w;
	uc AVGs[8];
	uc DIFs[8];
	...
	for (int h=2; h < height-1; h++){
		...
		for (w=2;w<=rowsize-2;w++){
			...
			avrg = row0[w]+row4[w];
			if (w < rowsize-1) avrg += row1[w+1]+row2[w+2]+row3[w+1];
			if (w>2)           avrg += row1[w-1]+row2[w-2]+row3[w-1];
			avrg = (w<2) ? avrg/5 : (w>rowsize-1) ? avrg/5 : avrg/8;

			diff = abs(avrg-row0[w]) + abs(avrg-row4[w]); //asmではabsを2つの大きい方-小さい方でやってる
			if (w < rowsize-1) diff += abs(avrg-row1[w+1]) + abs(avrg-row2[w+2]) + abs(avrg-row3[w+1]);
			if (w > 2)         diff += abs(avrg-row1[w-1]) + abs(avrg-row2[w-2]) + abs(avrg-row3[w-1]);
...
*/					
	...
	memcpy((BYTE*) dstp+dst_pitch, (BYTE*) row3, rowsize);
}
